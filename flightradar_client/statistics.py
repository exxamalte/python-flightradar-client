...
class Statistics:
    ...
    async def update_successful(self, updated_keys):
        """Record a successful update."""
        # 1. Update existing entries.
        for key in self._entries:
            if key in updated_keys:
                self._entries[key].update_successful()
            else:
                self._entries[key].update_unsuccessful()
        # 2. Add new entries.
        for key in updated_keys:
            if key not in self._entries:
                self._entries[key] = StatisticsData(True)

    async def update_unsuccessful(self):
        """Record an unsuccessful update."""
        for key in self._entries:
            # Always increase counter for total number of updates.
            self._entries[key].update_unsuccessful()
...
class StatisticsData:
    ...
    def __init__(self, successful):
        ...
        self._success = 1 if successful else 0
        self._total = 1
